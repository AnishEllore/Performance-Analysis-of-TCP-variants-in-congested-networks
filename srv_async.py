...
import select, zen_utils,socket
...
def serve(listener):
    ...
    for fd, event in all_events_forever(poll_object):
        ...
        # Incoming data: keep receiving until we see the suffix.
            ...
            print(i)
            i=i+1
            more_data = sock.recv(65535)
            if not more_data:  # end-of-file
                for id in sockets:
                    if sockets[id]!=listener and sockets[id]!=sock:
                        bytes_to_send[sockets[id]] = data
                        poll_object.modify(sockets[id], select.POLLOUT)
